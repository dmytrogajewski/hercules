...
// DSLProvider implements the UAST provider interface using DSL-based mappings.
...
// NewDSLProvider creates a new DSL-based provider with the given language and mapping rules.
func NewDSLProvider(lang *sitter.Language, langName string, rules []mapping.MappingRule) *DSLProvider {
	return &DSLProvider{
		language:       lang,
		langName:       langName,
		mappingRules:   rules,
		patternMatcher: mapping.NewPatternMatcher(lang),
	}
}

// Parse parses the given file content and returns the root UAST node.
func (p *DSLProvider) Parse(filename string, content []byte) (*node.Node, error) {
	parser := sitter.NewParser()
	parser.SetLanguage(p.language)
	tree, err := parser.ParseString(context.Background(), nil, content)
	if err != nil {
		return nil, fmt.Errorf("dsl provider: failed to parse: %w", err)
	}
	root := tree.RootNode()
	if root.IsNull() {
		return nil, errors.New("dsl provider: no root node")
	}
...
func (p *DSLProvider) Language() string {
	return p.langName
}

// SupportedLanguages returns the list of supported languages.
func (p *DSLProvider) SupportedLanguages() []string {
	return []string{p.langName}
}

// IsSupported checks if the given filename is supported by this provider.
func (p *DSLProvider) IsSupported(filename string) bool {
	// For now, support all files for the registered language
	// This can be enhanced with file extension checking
	return true
}
...
func (p *DSLProvider) createDSLNode(root sitter.Node, tree *sitter.Tree, content []byte) *DSLNode {
	return &DSLNode{
		Root:            root,
		Tree:            tree,
		Language:        p.langName,
...
func (dn *DSLNode) resolveInheritance(rule *mapping.MappingRule) *mapping.MappingRule {
	...
	if len(rule.UASTSpec.Props) > 0 {
		...
		for k, v := range rule.UASTSpec.Props {
			merged.UASTSpec.Props[k] = v
		}
	}
...
func (dn *DSLNode) shouldSkipEmptyFile(nodeType string, mappingRule *mapping.MappingRule, children []*node.Node) bool {
...
func (dn *DSLNode) extractSelfToken() string {
	if dn.Root.ChildCount() == 0 {
		return dn.extractNodeText(dn.Root)
	}
	return ""
...
func (dn *DSLNode) extractChildOrDescendantToken(source string) string {
	if dn.isChildToken(source) {
		return dn.extractChildToken(source)
	}
	if dn.isDescendantToken(source) {
		return dn.extractDescendantToken(source)
	}
	return ""
}

// isChildToken checks if the source is a child token
func (dn *DSLNode) isChildToken(source string) bool {
	_, ok := strings.CutPrefix(source, "child:")
	return ok
}

// extractChildToken extracts token from a child field
func (dn *DSLNode) extractChildToken(source string) string {
	after, _ := strings.CutPrefix(source, "child:")
	childNode := dn.Root.ChildByFieldName(after)
	if !childNode.IsNull() {
		return dn.extractNodeText(childNode)
	}
	return ""
}

// isDescendantToken checks if the source is a descendant token
func (dn *DSLNode) isDescendantToken(source string) bool {
	_, ok := strings.CutPrefix(source, "descendant:")
	return ok
}

// extractDescendantToken extracts token from a descendant
func (dn *DSLNode) extractDescendantToken(source string) string {
